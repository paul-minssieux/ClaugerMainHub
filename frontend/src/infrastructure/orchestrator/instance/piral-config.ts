...
export function getPiralConfig(): PiralInstanceConfig {
  ...
  const cacheOptions: PiletCacheOptions = {
    ...
    maxSize: 50, // 50MB max
    strategy: environment === 'development' ? 'network-first' : 'cache-first',
  ...
  const configs: Record<string, PiralInstanceConfig> = {
    development: {
      feedUrl: import.meta.env.VITE_PILETS_FEED_URL || 'http://localhost:3001/api/pilets',
      debug: true,
      strictMode: true,
      async: true,
      cacheOptions,
    ...
    staging: {
      feedUrl: import.meta.env.VITE_PILETS_FEED_URL || 'https://staging.clauger.com/api/pilets',
      debug: false,
      strictMode: true,
      async: true,
      cacheOptions,
    ...
    production: {
      feedUrl: import.meta.env.VITE_PILETS_FEED_URL || 'https://api.clauger.com/api/pilets',
      debug: false,
      strictMode: true,
      async: true,
      cacheOptions,
...
/**
 * Liste des pilets critiques à précharger
 */
export const criticalPilets = [
  'sidebar',
  'auth',
  'dashboard',
]